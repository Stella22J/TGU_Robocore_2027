/**
 * @file tracker.cpp
 * @brief 装甲目标跟踪器实现，负责在多帧检测之间维持稳定锁定目标
 */
...
Tracker::Tracker(const std::string& config_path, Solver& solver)
    : solver_{solver}, detect_count_(0), temp_lost_count_(0), state_{"lost"}, pre_state_{"lost"},
      last_timestamp_(std::chrono::steady_clock::now()),
      omni_target_priority_{ArmorPriority::fifth} {
    auto config = tools::load(config_path);
    enemy_color_ =
        (tools::read<std::string>(config, "enemy_color") == "red") ? Color::red : Color::blue;
    min_detect_count_ = tools::read<int>(config, "min_detect_count");
    max_temp_lost_count_ = tools::read<int>(config, "max_temp_lost_count");
    outpost_max_temp_lost_count_ = tools::read<int>(config, "outpost_max_temp_lost_count");
...
std::list<Target> Tracker::track(std::list<Armor>& armors, std::chrono::steady_clock::time_point t,
    ...
    // 过滤前哨站顶部装甲板
    // 可选颜色过滤逻辑，调试全颜色识别时可临时关闭
    // 前哨站姿态调试逻辑，比较两个候选俯仰角的重投影误差
    // 正俯仰角候选的重投影误差较小时认为姿态更可信
    // 负俯仰角候选用于排除前哨站姿态翻转
    // });
...
Tracker::track(const std::vector<omniperception::DetectionResult>& detection_queue,
