/**
 * @file yolo11.cpp
 * @brief YOLO11 OpenVINO后端实现，适配对应模型的输入尺寸和输出格式
 */
...
YOLO11::YOLO11(const std::string& config_path, bool debug)
    ...
    model_path_ = tools::read<std::string>(config, "yolo11_model_path");
...
std::list<Armor> YOLO11::parse(
    ...
    int frame_count) { // 每一行包含检测框、类别分数和关键点信息，解析顺序必须和模型导出一致
...
std::list<Armor> YOLO11::postprocess(double scale, cv::Mat& output, const cv::Mat& bgr_img,
                                     int frame_count) {
