/**
 * @file yolov5.cpp
 * @brief YOLOv5 OpenVINO后端实现，兼容输出中包含颜色和编号的模型格式
 */
...
YOLOV5::YOLOV5(const std::string& config_path, bool debug)
    ...
    model_path_ = tools::read<std::string>(config, "yolov5_model_path");
    ...
    use_traditional_ = tools::read<bool>(config, "use_traditional");
...
std::list<Armor> YOLOV5::parse(double scale, cv::Mat& output, const cv::Mat& bgr_img,
                               int frame_count) {
...
void YOLOV5::draw_detections(const cv::Mat& img, const std::list<Armor>& armors,
                             int frame_count) const {
    auto detection = img.clone();
    tools::draw_text(detection, fmt::format("[{}]", frame_count), {10, 30}, {255, 255, 255});
    for (const auto& armor : armors) {
        auto info = fmt::format("{:.2f} {} {} {}", armor.confidence, COLORS[armor.color],
...
std::list<Armor> YOLOV5::postprocess(double scale, cv::Mat& output, const cv::Mat& bgr_img,
                                     int frame_count) {
