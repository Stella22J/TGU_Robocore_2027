/**
 * @file yolov8.cpp
 * @brief YOLOv8 OpenVINO后端实现，适配定位和数字分类分离的检测流程
 */
...
YOLOV8::YOLOV8(const std::string& config_path, bool debug)
    ...
    model_path_ = tools::read<std::string>(config, "yolov8_model_path");
    device_ = tools::read<std::string>(config, "device");
    binary_threshold_ = tools::read<double>(config, "threshold");
    min_confidence_ = tools::read<double>(config, "min_confidence");
    const auto& roi_config = tools::read_table(config, "roi");
    int x = 0, y = 0, width = 0, height = 0;
    x = tools::read<int>(roi_config, "x");
    y = tools::read<int>(roi_config, "y");
    width = tools::read<int>(roi_config, "width");
    height = tools::read<int>(roi_config, "height");
    use_roi_ = tools::read<bool>(config, "use_roi");
...
std::list<Armor> YOLOV8::parse(double scale, cv::Mat& output, const cv::Mat& bgr_img,
                               int frame_count) {
    // 每一行包含检测框、类别分数和关键点信息，解析顺序必须和模型导出一致
...
void YOLOV8::draw_detections(const cv::Mat& img, const std::list<Armor>& armors,
...
void YOLOV8::sort_keypoints(std::vector<cv::Point2f>& keypoints) {
    ...
    keypoints[0] = top_points[0];    // 左上角点，PnP要求角点顺序固定
    keypoints[1] = top_points[1];    // 右上角点，PnP要求角点顺序固定
    keypoints[2] = bottom_points[1]; // 右下角点，PnP要求角点顺序固定
    keypoints[3] = bottom_points[0]; // 左下角点，PnP要求角点顺序固定
}

std::list<Armor> YOLOV8::postprocess(double scale, cv::Mat& output, const cv::Mat& bgr_img,
                                     int frame_count) {
