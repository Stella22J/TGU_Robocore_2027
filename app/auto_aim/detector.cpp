/**
 * @file detector.cpp
 * @brief 传统灯条装甲板检测实现，作为神经网络之外的几何检测和角点修正路径
 */
...
Detector::Detector(const std::string& config_path, bool debug)
    ...
    threshold_ = tools::read<double>(config, "threshold");
    max_angle_error_ = tools::read<double>(config, "max_angle_error") /
                       57.3; // 配置使用角度，控制和预测统一使用弧度
    min_lightbar_ratio_ = tools::read<double>(config, "min_lightbar_ratio");
    max_lightbar_ratio_ = tools::read<double>(config, "max_lightbar_ratio");
    min_lightbar_length_ = tools::read<double>(config, "min_lightbar_length");
    min_armor_ratio_ = tools::read<double>(config, "min_armor_ratio");
    max_armor_ratio_ = tools::read<double>(config, "max_armor_ratio");
    max_side_ratio_ = tools::read<double>(config, "max_side_ratio");
    min_confidence_ = tools::read<double>(config, "min_confidence");
    max_rectangular_error_ = tools::read<double>(config, "max_rectangular_error") /
                             57.3; // 配置使用角度，控制和预测统一使用弧度
...
cv::Mat Detector::get_pattern(const cv::Mat& bgr_img, const Armor& armor) const {
    // 延长灯条获得装甲板角点
    // 1.125来自装甲板高度和灯条长度的比例，用于在裁剪数字区域时留出足够上下边界
...
ArmorType Detector::get_type(const Armor& armor) {
    ...
    if (armor.ratio > 3.0) {
        // 调试几何筛选过程，正式运行时避免高频日志影响实时性
        // 调试大装甲板类型判断，便于检查比例阈值是否过严
        return ArmorType::big;
    }

    if (armor.ratio < 2.5) {
        // 调试几何筛选过程，正式运行时避免高频日志影响实时性
        // 调试小装甲板类型判断，便于检查比例阈值是否过松
        return ArmorType::small;
    }

    // 调试几何筛选过程，正式运行时避免高频日志影响实时性"[Detector] get armor type by name: {}",
...
void Detector::show_result(const cv::Mat& binary_img, const cv::Mat& bgr_img,
    ...
    for (const auto& lightbar : lightbars) {
        auto info = fmt::format("{:.1f} {:.1f} {:.1f} {}", lightbar.angle_error * 57.3,
                                lightbar.ratio, lightbar.length, COLORS[lightbar.color]);
...
void Detector::lightbar_points_corrector(Lightbar& lightbar, const cv::Mat& gray_img) const {
    ...
    constexpr float ROI_SCALE = 0.07;    // ROI扩展比例，给灯条边缘留出余量以提高角点修正稳定性
    ...
    const cv::Point2f centroid(moments.m10 / moments.m00 + roi_box.x,
    ...
    const auto find_corner = [&](int direction) -> cv::Point2f {
        ...
        for (int i_offset = -half_width; i_offset <= half_width; ++i_offset) {
            // 计算搜索起点
            cv::Point2f start_point(centroid.x + lightbar.length * SEARCH_START * dx + i_offset,
            ...
            for (float step = 0; step < search_length; ++step) {
                ...
                // 边界检查
                if (cur_point.x < 0 || cur_point.x >= gray_img.cols || cur_point.y < 0 ||
                ...
                // 计算亮度差（使用双线性插值提升精度）
                const auto prev_val =
                    gray_img.at<uchar>(cv::Point2i(cur_point - cv::Point2f(dx, dy)));
