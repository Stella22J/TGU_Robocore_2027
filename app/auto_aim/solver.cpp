/**
 * @file solver.cpp
 * @brief 装甲板位姿解算实现，负责把像素角点转换为云台和世界坐标系下的三维量
 */
...
namespace auto_aim {
constexpr double LIGHTBAR_LENGTH = 56e-3;    // 单位为米，PnP模型必须和实际装甲板尺寸一致
constexpr double BIG_ARMOR_WIDTH = 230e-3;   // 单位为米，PnP模型必须和实际装甲板尺寸一致
constexpr double SMALL_ARMOR_WIDTH = 135e-3; // 单位为米，PnP模型必须和实际装甲板尺寸一致
...
Solver::Solver(const std::string& config_path) : R_gimbal2world_(Eigen::Matrix3d::Identity()) {
    ...
    auto R_gimbal2imubody_data = tools::read_array<double>(config, "R_gimbal2imubody");
    auto R_camera2gimbal_data = tools::read_array<double>(config, "R_camera2gimbal");
    auto t_camera2gimbal_data = tools::read_array<double>(config, "t_camera2gimbal");
    R_gimbal2imubody_ = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>(R_gimbal2imubody_data.data());
    R_camera2gimbal_ = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>(R_camera2gimbal_data.data());
    t_camera2gimbal_ = Eigen::Matrix<double, 3, 1>(t_camera2gimbal_data.data());

    auto camera_matrix_data = tools::read_array<double>(config, "camera_matrix");
    auto distort_coeffs_data = tools::read_array<double>(config, "distort_coeffs");
...
void Solver::optimize_yaw(Armor& armor) const {
    ...
    constexpr double SEARCH_RANGE = 140; // 单位为度，搜索范围来自调试经验，进入计算前会转换为弧度
...
double Solver::SJTU_cost(const std::vector<cv::Point2f>& cv_refs,
    ...
    for (std::size_t i = 0u; i < size; ++i) {
        ...
        // 平方可能是为了配合 sin 和 cos
        // 弧度差代价（0 度左右占比应该大）
        double cost_i = tools::square(pixel_dis * std::sin(inclined)) +
                        tools::square(angular_dis * std::cos(inclined)) *
                            2.0; // 按倾斜角放大像素误差，降低斜视装甲板对优化的干扰
        // 重投影像素误差越大，越相信斜率
