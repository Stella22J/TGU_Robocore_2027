 ...
 * @brief 异步OpenVINO检测器实现，提交图像后可与其他流程并行等待推理结果
 */
...
MultiThreadDetector::MultiThreadDetector(const std::string& config_path, bool debug)
    ...
    auto model_path = tools::read<std::string>(config, yolo_name + "_model_path");
    ...
    input.tensor()
        ...
        .set_shape({1, 640, 640, 3}) // TODO: 输入尺寸应和模型导出尺寸保持一致
        .set_layout("NHWC")
