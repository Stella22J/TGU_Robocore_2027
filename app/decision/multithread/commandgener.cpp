/**
 * @file commandgener.cpp
 * @brief 高频命令生成实现，在独立线程中持续把最新目标状态转换为下位机命令
 */
...
CommandGener::CommandGener(auto_aim::Shooter& shooter, auto_aim::Aimer& aimer, io::CBoard& cboard,
                           tools::Plotter& plotter, bool debug)
    : shooter_(shooter), aimer_(aimer), cboard_(cboard), plotter_(plotter), stop_(false),
      debug_(debug) {
...
void CommandGener::push(const std::list<auto_aim::Target>& targets,
    ...
    while (!stop_) {
        ...
        if (input) {
            ...
            command.horizon_distance =
                input->targets_.empty()
                    ? 0
                    : std::sqrt(tools::square(input->targets_.front().ekf_x()[0]) +
