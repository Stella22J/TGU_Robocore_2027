/**
 * @file decider.cpp
 * @brief 全向感知目标决策实现，根据多相机检测结果生成粗略云台转向命令
 */
...
Decider::Decider(const std::string& config_path) : detector_(config_path), count_(0) {
    ...
    img_width_ = tools::read<double>(config, "image_width");
    img_height_ = tools::read<double>(config, "image_height");
    fov_h_ = tools::read<double>(config, "fov_h");
    fov_v_ = tools::read<double>(config, "fov_v");
    new_fov_h_ = tools::read<double>(config, "new_fov_h");
    new_fov_v_ = tools::read<double>(config, "new_fov_v");
    enemy_color_ = (tools::read<std::string>(config, "enemy_color") == "red")
                       ? auto_aim::Color::red
                       : auto_aim::Color::blue;
    mode_ = tools::read<int>(config, "mode");
}

io::Command Decider::decide(auto_aim::YOLO& yolo, const Eigen::Vector3d& gimbal_pos,
...
Eigen::Vector4d Decider::get_target_info(const std::list<auto_aim::Armor>& armors,
