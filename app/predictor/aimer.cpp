/**
 * @file aimer.cpp
 * @brief 弹道瞄准预测实现，综合目标运动、系统延迟和弹道下坠生成云台角度
 */
...
Aimer::Aimer(const std::string& config_path)
    ...
    yaw_offset_ =
        tools::read<double>(config, "yaw_offset") / 57.3; // 配置使用角度，控制和预测统一使用弧度
    pitch_offset_ =
        tools::read<double>(config, "pitch_offset") / 57.3; // 配置使用角度，控制和预测统一使用弧度
    comming_angle_ =
        tools::read<double>(config, "comming_angle") / 57.3; // 配置使用角度，控制和预测统一使用弧度
    leaving_angle_ =
        tools::read<double>(config, "leaving_angle") / 57.3; // 配置使用角度，控制和预测统一使用弧度
    high_speed_delay_time_ = tools::read<double>(config, "high_speed_delay_time");
    low_speed_delay_time_ = tools::read<double>(config, "low_speed_delay_time");
    decision_speed_ = tools::read<double>(config, "decision_speed");
    auto left_yaw_offset = tools::read_optional<double>(config, "left_yaw_offset");
    auto right_yaw_offset = tools::read_optional<double>(config, "right_yaw_offset");
...
io::Command Aimer::aim(std::list<Target> targets, std::chrono::steady_clock::time_point timestamp,
    ...
    // 补偿detector和tracker消耗的时间，aimer计算耗时相对较小因此忽略
    ...
    if (!aim_point0.valid) {
        // 调试几何筛选过程，正式运行时避免高频日志影响实时性"Invalid aim_point0");
        return {false, false, 0, 0};
    }
    ...
    // 迭代求解飞行时间，最多10次，避免远距离目标飞行时间估计偏小
