...
CBoard::CBoard(const std::string& config_path)
    : mode(Mode::idle), shoot_mode(ShootMode::left_shoot), bullet_speed(0), ft_angle(0),
      queue_(5000),
      can_(read_toml(config_path), std::bind(&CBoard::callback, this, std::placeholders::_1)) {
    // callback可能早于CBoard构造函数完成运行,所以queue_必须在can_之前初始化
...
Eigen::Quaterniond CBoard::imu_at(std::chrono::steady_clock::time_point timestamp) {
    // 如果当前data_behind_仍早于目标时间,则先将其作为data_ahead_
    if (data_behind_.timestamp < timestamp)
        data_ahead_ = data_behind_;

    // 从IMU队列中继续取数据,直到找到第一帧晚于目标时间的IMU数据
    while (true) {
        queue_.pop(data_behind_);
